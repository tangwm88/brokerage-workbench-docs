...
tasks = [
    ('1', '产品创设',
     '触发：员工/外部投资经理有产品设计意向。调用"资管产品设计原子模型"（配置/风险收益/期限三维度）+"同业比对原子模型"+"收益测算原子模型"生成方案。输出：标准化产品方案。规则：全员均可参与产品设计，投资经理举手运行'),
    ('2', '资金募集',
     '触发：产品方案确认、需要募集资金。调用"客户分类原子模型"+"客户与产品匹配规则原子模型"+"内容推介生成原子模型"+"资管产品合规模型"。输出：目标客户清单+推介内容+分派/抢单任务。规则：有经纪关系优先派发，无关系全员抢单'),
    ('3', '投资运作',
     '触发：产品成立、进入投资运作期。调用"资管产品设计原子模型"确认投资范围+"资管产品合规模型"持续监测。输出：投资决策记录+风险监测报告+净值更新。规则：AI只做监测、留痕、报警，不干预投资决策'),
    ('4', '渠道代销',
     '触发：产品开放代销需求。调用"渠道准入原子模型"（区分已准入机构和新拓展机构）+"资管产品合规模型"。输出：代销方案+渠道匹配清单。规则：已准入和新拓展分开处理，不混为一谈'),
    ('5', '售后与持续服务',
     '触发：产品运行期间定期/事件驱动。调用"产品售后动作原子模型"（定期报告/净值波动/重大变化/大额赎回）。输出：客户沟通工单+内部报告。规则：事件触发自动派发，按响应时效考核'),
    ('6', '监督全过程管控',
     '触发：贯穿所有任务，独立运行。调用"监督模型"+"资管产品合规模型"+"权限模型"。输出：进度管控+质量检查+合规留痕+异常报警+推进督促+计量核算。规则：只留痕报警督促，不干预业务执行'),
    ('7', '贡献计量与收入分配',
     '触发：任务闭环（产品成立/募集完成/交易达成/服务周期结束）。调用"员工贡献计量分配原子模型"+"总绩效计量原子模型"+"跟投评估模型"。输出：各参与人贡献度+收入分配结果。规则：机器完成的不计量，只拆核心链条，中后台不参与分成，当期+递延'),
]
