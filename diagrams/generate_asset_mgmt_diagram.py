...
sections_right = [
    {
        ...
        'models': [
            ...
            },
            {
                ...
                'tag': '7项',
                'items': [
                    ('宣传合规', '不得公开宣传收益率/承诺保本/极端词汇'),
                    ('合格投资者', '金融资产≥300万/年均收入≥50万/经验满2年'),
                    ('风险揭示', '充分揭示风险/签署风险揭示书'),
...
tasks = [
    ('1', '发起创设/引入意向',
     '发起产品创设意向，来源包括：员工发起、外部投资经理提交、资金方需求、委外确认。生成标准化任务卡片，调用"任务组管理模型"组建任务组，并发调用客户分类原子模型/客户合作原子模型/持续服务原子模型做初步匹配，所有意向不判断可行性全部进入引擎'),
    ('2', '生成产品方案',
     '并发调用"资管产品设计原子模型"、"同业比对原子模型"、"收益测算原子模型"生成产品方案，调用"投资管理匹配原子模型"做六维评价，调用"资管产品合规模型"、"准入标准费率原子模型"、"总绩效计量原子模型"完成合规校验与费率确认，不通过给调优建议不拒绝'),
    ('3', '产品准入与签约',
     '调用"客户分类原子模型"、"客户与产品匹配规则原子模型"进行自动撮合匹配找到目标客户，调用"内容推介生成原子模型"生成推介内容，经"资管产品合规模型"验证后，有经纪关系的派发给客户经理，无关系的全平台抢单'),
    ('4', '资金募集',
     '调用"客户合作原子模型"区分已准入机构和新拓展机构，已准入的直接办产品准入，新拓展的先办机构准入再办产品准入，两种情形不混在一起，完成后签署合作协议和产品销售协议'),
    ('5', '投资运作',
    ...
    ('7', '监督全过程管控',
     '监督模型贯穿全程并发运行，进度管控、质量检查、合规留痕、异常报警、推进督促、计量核算，调用"资管产品合规模型"留痕，只留痕报警督促不干预业务执行'),
    ('8', '贡献计量与收入分配',
...
footer_text = '资产管理自动驾驶平台 v3.0 | 一套方法论、一套模型、套不同产品特征 | 开放→聚合→机制→产品'
