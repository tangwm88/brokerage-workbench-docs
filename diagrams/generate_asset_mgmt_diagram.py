...
tasks = [
    ...
    ('4', '渠道代销',
     '调用"渠道准入原子模型"区分已准入机构和新拓展机构，已准入的直接办产品准入，新拓展的先办机构准入再办产品准入，两种情形不混在一起，完成后产品上架'),
    ('5', '投资运作',
     '确认投资经理或挂名投资经理，调用"资管产品设计原子模型"确认投资范围与限制，调用"资管产品合规模型"持续监测，净值异常只报警不干预投资决策'),
    ('6', '售后持续服务',
     '调用"产品售后动作原子模型"，事件驱动并发处理定期报告、净值波动沟通(当日3%/历史10%)、重大变化沟通、大额赎回(月减30%)、客户咨询响应，自动生成售后工单'),
    ('7', '监督全过程管控',
...
footer_text = '资产管理自动驾驶平台 v2.1 | 一套方法论、一套模型、套不同产品特征 | 开放→聚合→机制→产品 | 对标贝莱德、彭博'
